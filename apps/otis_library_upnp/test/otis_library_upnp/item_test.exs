defmodule Otis.Library.UPNP.ItemTest do
  ...
  test "generates valid ids for source lookup" do
    item = %Item{
      id: "1$7$F$7",
      device_id:
        "uuid:4d696e69-444c-164e-9d41-001c42fc0db6::urn:schemas-upnp-org:device:MediaServer:1"
    }

    assert Otis.Library.Source.id(item) ==
             "uuid%3A4d696e69-444c-164e-9d41-001c42fc0db6%3A%3Aurn%3Aschemas-upnp-org%3Adevice%3AMediaServer%3A1/1%247%24F%247"
  end
  ...
  test "returns correct type & extension info" do
    media = %Media{
      bitrate: 256_000,
      channels: 2,
      duration: "0:02:45.186",
      info:
        "http-get:*:audio/mp4:DLNA.ORG_PN=AAC_ISO_320;DLNA.ORG_OP=01;DLNA.ORG_CI=0;DLNA.ORG_FLAGS=01700000000000000000000000000000",
      sample_freq: 44_100,
      size: 6_403_418,
