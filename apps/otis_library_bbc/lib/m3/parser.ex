defmodule M3.Parser do
  ...
  defp parse_line(url, parser) do
    ...
    parser
    |> append_media(%Media{url: to_string(uri), duration: duration, filename: filename})
    |> reset_extinf()
  end
  ...
  defp set_stream_inf(
         parser,
         %{bandwidth: _bandwidth, codecs: _codecs, program_id: _program_id} = stream_inf
       ) do
    %Parser{parser | type: %M3.Playlist.Variant{}, streaminf: stream_inf}
  end

  defp set_stream_inf(parser, %{
         "BANDWIDTH" => bandwidth,
         "CODECS" => codecs,
         "PROGRAM-ID" => program_id
       }) do
    set_stream_inf(parser, %{
      bandwidth: String.to_integer(bandwidth),
      codecs: parse_codecs(String.split(codecs, ",")),
