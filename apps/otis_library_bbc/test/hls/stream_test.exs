defmodule HLS.StreamTest do
  ...
  test "returns the highest & lowest bandwidth variants", context do
    ...
    assert %{url: "http://bbc.io/radio4_high.m3u8", bandwidth: 339_200} ==
             Map.take(data, [:url, :bandwidth])

    data = Stream.lowest(context.stream)

    assert %{url: "http://bbc.io/radio4_low.m3u8", bandwidth: 101_760} ==
             Map.take(data, [:url, :bandwidth])
  end
  ...
  test "can provide the next lowest bandwidth stream", context do
    ...
    assert %{url: "http://bbc.io/radio4_med.m3u8", bandwidth: 135_680} ==
             Map.take(data, [:url, :bandwidth])
  ...
  test "can resolve a variant playlist to a live one", context do
    ...
    assert %M3.Media{duration: 6, filename: "no desc", url: "http://bbc.io/high/226201865.ts"} ==
             file
  end
end
