defmodule Otis.Receivers do
  ...
  when is_binary(id) do
    ...
    case receiver(id) do
      ...
      _error ->
        Otis.Events.notify({:receiver_volume_change, [id, volume]})
    end
    {:ok, volume}
  end
  ...
  def rename(id, name) when is_binary(id) do
    Otis.Events.notify({:receiver_rename, [id, name]})
  end
  ...
  def handle_call({:attach, receiver_id, channel_id}, _from, state) do
    ...
    Otis.Events.notify({:reattach_receiver, [receiver_id, channel_id, receiver]})
    {:reply, :ok, state}
  end
  ...
  def start_valid_receiver(state, receiver, true) do
    Otis.Events.notify({:receiver_connected, [receiver.id, receiver]})
    state
  end
  ...
  def disable_zombie_receiver(state, receiver, true) do
    Otis.Events.notify({:receiver_disconnected, [receiver.id, receiver]})
    state
  end
  ...
  def remove_dead_receiver(state, receiver, true) do
    Otis.Events.notify({:receiver_offline, [receiver.id, receiver]})
  ...
  defp mute_receiver({:ok, receiver}, id, muted, state) do
    ...
    Otis.Events.notify({:receiver_muted, [id, muted]})
    state
  end
end
