defmodule Otis.State.Setting do
  ...
  defp _delete(app, ns, key) when is_binary(app) and is_binary(ns) and is_binary(key) do
    Setting
    |> where(application: ^app, namespace: ^ns, key: ^key)
    |> Repo.delete_all
  end
  defp _delete(app, ns, key), do: _delete(to_string(app), to_string(ns), to_string(key))

  defp _put!(app, ns, key, value) do
