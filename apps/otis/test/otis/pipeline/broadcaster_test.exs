defmodule Test.Otis.Pipeline.Broadcaster do
  ...
  setup do
    ...
    {:ok,
     channel: channel_record, channel_id: @channel_id, receivers: [r1, r2], mocks: [mock1, mock2]}
  end
  ...
  test "broadcaster does a flood send of receivers on start", context do
    ...
    packet_time = fn n ->
      time + @receiver_latency + config.base_latency_ms * 1000 +
        n * config.packet_duration_ms * 1_000
    end
  ...
  test "clock is started after source is loaded", context do
    ...
    packet_time = fn t0, n ->
      t0 + @receiver_latency + config.base_latency_ms * 1000 +
        n * config.packet_duration_ms * 1_000
    end
